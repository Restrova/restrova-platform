...
export function formatCurrency(value, { locale = DEFAULT_LOCALE, currency = DEFAULT_CURRENCY, fallback = FALLBACK_DISPLAY, ...options } = {}) {
  const number = toNumber(value);
  if (number === null) return fallback;
  return safeFormat(() => new Intl.NumberFormat(normalizeLocale(locale), {
    style: "currency",
    currency,
...
export function formatDate(value, { locale = DEFAULT_LOCALE, timezone = DEFAULT_TIMEZONE, fallback = FALLBACK_DISPLAY, ...options } = {}) {
  if (isMissing(value)) return fallback;
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return fallback;
  return safeFormat(() => new Intl.DateTimeFormat(normalizeLocale(locale), {
    timeZone: timezone,
    dateStyle: "medium",
    ...options
  }).format(date));
...
export function formatDateTime(value, { locale = DEFAULT_LOCALE, timezone = DEFAULT_TIMEZONE, fallback = FALLBACK_DISPLAY, ...options } = {}) {
  if (isMissing(value)) return fallback;
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return fallback;
  return safeFormat(() => new Intl.DateTimeFormat(normalizeLocale(locale), {
