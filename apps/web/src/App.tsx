...
export function App() {
  ...
  if (authenticated) return <main className="dashboard"><header><div><p className="eyebrow">SECURE SESSION</p><h1 data-testid="decision-workspace">Decision workspace</h1><p>Import restaurant data to calculate the next measurable action.</p></div><button onClick={() => setAuthenticated(false)}>Sign out</button></header><section className="import-card"><h2>Start a decision cycle</h2><p>Upload a CSV or XLSX export from your restaurant system. The source system remains authoritative.</p><label className="upload">Choose sales export<input data-testid="sales-upload" type="file" accept=".csv,.xlsx,.xls" /></label></section><section className="metrics"><article><span>Net sales</span><strong>—</strong><small>Awaiting validated import</small></article><article><span>Leakage</span><strong>—</strong><small>Calculated deterministically</small></article><article><span>Data quality</span><strong>Missing</strong><small>Import required</small></article></section><section className="decision-card"><p className="eyebrow">DECISION LOOP</p><h2>No issue ranked yet</h2><p>After validation, the engine will rank one issue, show evidence, and recommend one practical action.</p><div className="reviews"><span>7-day review <b>Not scheduled</b></span><span>14-day review <b>Not scheduled</b></span></div></section></main>;
