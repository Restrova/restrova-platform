...
- Currency conversion is never implicit. Organization reports aggregate only entries in that organization's currency; UI formatting uses the ISO currency's minor-unit precision (for example CNY 2, JPY 0, and BHD 3).
...
- Gross margin = gross profit ÷ revenue when revenue is positive.
- Contribution profit = gross profit − packaging − delivery commissions.
- Contribution margin = contribution profit ÷ revenue when revenue is positive.
...
- Net margin = net profit ÷ revenue when revenue is positive.
...
Money outputs remain integer minor units. Margins use integer basis points (`10000` = 100%) and are rounded to the nearest basis point, half away from zero. Per-order values are rounded to the nearest minor unit using the same rule. Margins are `null` when revenue is zero or negative because a percentage would be commercially misleading.
...
Tax treatment is explicitly `not_modeled`: the ledger rejects an unsupported tax category, the engine does not estimate or deduct tax, and net profit remains equal to operating profit. This prevents an apparently precise tax result from being invented before a versioned tax policy exists.
...
- `GET /api/financial/calculate` returns the deterministic `3.7-v1` metrics with optional `branchId`, `from`, and `to` filters.
