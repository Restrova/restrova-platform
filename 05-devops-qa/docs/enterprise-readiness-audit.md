...
| Area | Current state |
| --- | --- |
| Repository architecture | npm/pnpm workspace with team folders `02-backend`, `03-frontend`, `04-data-ai`, `05-devops-qa`. Some tooling remains root-level, which is correct for package/deployment discovery. |
...
| ER-019 | P1 | Deployment | `05-devops-qa/Dockerfile`, `package.json`, `pnpm-lock.yaml` | Repository is pnpm-based, but Docker uses `npm install`; lockfile reproducibility is not guaranteed. | Production dependency tree can differ from tested local dependency tree. | Use Corepack + `pnpm install --frozen-lockfile` in Docker and CI. | No. |
...
| ER-035 | P2 | Developer experience | `package.json`, `pnpm-workspace.yaml`, `05-devops-qa/Dockerfile` | Docs show both `npm` and `pnpm`; workspace is pnpm-configured; Docker uses npm. | New team members can install different dependency graphs or run different scripts. | Standardize on pnpm or npm everywhere; document exact Node/package-manager versions. | No. |
...
22. Developer experience: mixed npm/pnpm instructions and no one-command full validation.
