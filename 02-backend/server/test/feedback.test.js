...
test("feedback table enforces one expert judgment per assistant message", () => {
  ...
  const session = db
    .prepare("INSERT INTO chat_sessions(restaurant_id,title) VALUES (?,?)")
    .run(restaurant.id, "Feedback test");
  const message = db
    .prepare("INSERT INTO chat_messages(session_id,role,content) VALUES (?,'assistant',?)")
    .run(session.lastInsertRowid, "Test answer");
  try {
    db.prepare(
      "INSERT INTO answer_feedback(restaurant_id,session_id,message_id,question,original_answer,rating,correct_tools) VALUES (?,?,?,?,?,?,?)"
    ).run(
      restaurant.id,
      ...
      "Test answer",
      "approved",
      JSON.stringify(["get_daily_sales"])
    );
    assert.throws(
      () =>
        db
