...
export function createOwnerIdentity(email, passwordHash, name) {
  const id = Number(
    db.prepare("INSERT INTO owners(email,password_hash,name) VALUES (?,?,?)").run(email, passwordHash, name)
      .lastInsertRowid
  );
...
export function upsertOrganizationUser(organizationId, ownerId, role, branchId) {
  db.prepare(
    "INSERT INTO organization_users(organization_id,owner_id,role,branch_id) VALUES (?,?,?,?) ON CONFLICT(organization_id,owner_id) DO UPDATE SET role=excluded.role,branch_id=excluded.branch_id"
...
export function updateMembershipRole(membershipId, role, branchId) {
  db.prepare("UPDATE organization_users SET role=?,branch_id=? WHERE id=?").run(
    role,
