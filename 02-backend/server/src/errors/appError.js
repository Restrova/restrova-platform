...
export const rateLimited = (message = "Too many requests. Please try again later.") => new AppError(ErrorCodes.RATE_LIMITED, message, 429);
export const internalError = (cause) => new AppError(ErrorCodes.INTERNAL_ERROR, "Unable to complete request", 500, { expose: false, cause });

export function normalizeError(error) {
  if (error instanceof AppError) return error;
  if (error?.code && error?.status && Object.values(ErrorCodes).includes(error.code)) return new AppError(error.code, error.message || "Unable to complete request", error.status);
  ...
  if (error instanceof SyntaxError && error.status === 400 && "body" in error) return validationError("Malformed JSON body");
  ...
  if (/Preview this CSV|branch is required|Unsupported import|Missing required columns|must be/i.test(error?.message || "")) return validationError(error.message);
