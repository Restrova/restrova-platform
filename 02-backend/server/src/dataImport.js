...
const rowKey = (type, branchId, row) =>
  crypto.createHash("sha256").update(JSON.stringify({ type, branchId, row })).digest("hex");
    ...
    rows.forEach((row, index) => {
      ...
      if (type === "orders") {
        ...
        const items =
          ...
          JSON.stringify([
            {
              name: row.item_name || "Imported sale",
              quantity,
              price: +(totalPrice / quantity).toFixed(4),
              cost: +(totalCost / quantity).toFixed(4)
            }
          ]);
        try {
          JSON.parse(items);
        } catch {
          throw new Error(`Row ${line}: items must be valid JSON when provided.`);
        }
        const result = db
          .prepare(
            "INSERT OR IGNORE INTO orders(restaurant_id,branch_id,items,total_price,cost,discount,commission,other_cost,created_at,source_key) VALUES (?,?,?,?,?,?,?,?,?,?)"
          )
          .run(
            restaurantId,
            branchId,
            ...
            totalPrice,
            totalCost,
            number(row.discount || 0, "discount", line),
            number(row.commission || 0, "commission", line),
            number(row.other_cost || 0, "other_cost", line),
            date(row.created_at, "created_at", line),
            sourceKey
          );
        if (!result.changes) {
          skipped_duplicates++;
        ...
        const result = db
          .prepare(
            "INSERT OR IGNORE INTO refunds(restaurant_id,branch_id,order_id,amount,reason,created_at,source_key) VALUES (?,?,?,?,?,?,?)"
          )
          .run(
            restaurantId,
            branchId,
            row.order_id ? number(row.order_id, "order_id", line) : null,
            number(row.amount, "amount", line),
            row.reason || null,
            date(row.created_at, "created_at", line),
            sourceKey
          );
        ...
        const existing = db
          ...
          .get(restaurantId, row.name);
        if (existing)
          db.prepare("UPDATE menu_items SET price=?,cost=?,active=? WHERE id=?").run(
            number(row.price, "price", line),
            number(row.cost, "cost", line),
            row.active?.toLowerCase() === "false" ? 0 : 1,
            existing.id
          );
        else
        ...
        const existing = db
          .prepare("SELECT id FROM inventory WHERE restaurant_id=? AND branch_id=? AND lower(item_name)=lower(?)")
          .get(restaurantId, branchId, row.item_name);
        if (existing)
          db.prepare("UPDATE inventory SET quantity=?,threshold=? WHERE id=?").run(
            number(row.quantity, "quantity", line),
            number(row.threshold, "threshold", line),
            existing.id
          );
        else
          db.prepare(
        ...
        const start = date(row.start_at, "start_at", line);
        const end = date(row.end_at, "end_at", line);
        if (end <= start) throw new Error(`Row ${line}: end_at must be after start_at.`);
        db.prepare(
          "INSERT INTO staff_shifts(restaurant_id,branch_id,employee_name,role,start_at,end_at,hourly_rate) VALUES (?,?,?,?,?,?,?)"
        ).run(
          restaurantId,
          branchId,
          row.employee_name,
          ...
          number(row.hourly_rate, "hourly_rate", line)
        );
      }
      imported++;
    });
...
export function dataConnectionStatus(restaurantId, branchId) {
  const count = (table) => {
    const sql = branchId
      ? `SELECT count(*) count FROM ${table} WHERE restaurant_id=? AND branch_id=?`
      : `SELECT count(*) count FROM ${table} WHERE restaurant_id=?`;
